...
(async()=>{
  try{
    ...
    assert.ok(await evaluate(solo,'JSON.parse(localStorage.getItem(storeKey())).best>0'),'ghost persists');
    ...
    await evaluate(solo,"document.querySelector('[data-track=alpine]').click()");assert.equal(await evaluate(solo,'selectedTrack'),'alpine');assert.equal(await evaluate(solo,'local.trackId'),'alpine');assert.ok(await evaluate(solo,"$('nav-track').textContent.includes('ALPINE')"));await screenshot(solo,'alpine.png');
    await evaluate(solo,"document.querySelector('[data-track=sunset]').click()");assert.equal(await evaluate(solo,'renderer.trackId'),'sunset');await screenshot(solo,'sunset.png');
    await evaluate(solo,"document.querySelector('[data-track=metro]').click()");assert.equal(await evaluate(solo,'renderer.trackId'),'metro');assert.ok(await evaluate(solo,"$('nav-track').textContent.includes('NEON')"));await screenshot(solo,'metro.png');
    await evaluate(solo,"$('setup-downforce').value='high';$('setup-downforce').dispatchEvent(new Event('input'));$('setup-compound').value='soft';$('setup-compound').dispatchEvent(new Event('input'));$('setup-brake-bias').value='62';$('setup-brake-bias').dispatchEvent(new Event('input'))");assert.equal(await evaluate(solo,'local.setup.downforce'),'high');assert.equal(await evaluate(solo,'local.setup.compound'),'soft');assert.equal(await evaluate(solo,'local.setup.brakeBias'),62);assert.ok(await evaluate(solo,"localStorage.getItem('apex-setup').includes('high')"));
    await evaluate(solo,"document.querySelector('[data-track=emerald]').click()");assert.equal(await evaluate(solo,'renderer.trackId'),'emerald');await screenshot(solo,'emerald.png');
    ...
    await rpc('Emulation.setTouchEmulationEnabled',{enabled:true},mobile);const gas=await evaluate(mobile,"(()=>{const b=document.querySelector('[data-key=up]');b.scrollIntoView({block:'center'});const r=b.getBoundingClientRect();return {x:r.x+r.width/2,y:r.y+r.height/2};})()");await rpc('Input.dispatchTouchEvent',{type:'touchStart',touchPoints:[{x:gas.x,y:gas.y}]},mobile);await pause(900);assert.ok(await evaluate(mobile,'local.speed>40'),'touch throttle accelerates');await rpc('Input.dispatchTouchEvent',{type:'touchEnd',touchPoints:[]},mobile);assert.equal(await evaluate(mobile,'!!keys.up'),false,'touch release stops input');
    await evaluate(solo,"$('name').value='Host';$('race-laps').value='10';$('host').click()");await until(()=>evaluate(solo,'session!==null'),'host');const code=await evaluate(solo,'room.code');
    ...
    await evaluate(solo,"$('start').click()");await until(()=>evaluate(mobile,"room.status==='racing'"),'race stream');assert.equal(await evaluate(mobile,'room.laps'),10);assert.equal(await evaluate(mobile,'selectedTrack'),'emerald');assert.equal(await evaluate(solo,"room.players.find(p=>p.name==='Host').setup.downforce"),'high');await screenshot(solo,'multiplayer.png');
    ...
    console.log('PASS: five circuit renders, persisted garage setup, driving, steering, four-wheel physics, ghost storage, mobile layout, host/join track and setup sync, 10-lap countdown, remote movement, reset and host transfer.');
