...
(async()=>{
  try{
    ...
    await evaluate(solo,"$('start').click()");await until(()=>evaluate(mobile,"room.status==='racing'"),'race stream');assert.equal(await evaluate(mobile,'room.laps'),10);assert.equal(await evaluate(mobile,'selectedTrack'),'sunset');await screenshot(solo,'multiplayer.png');
    ...
    console.log('PASS: three circuit renders, driving, steering, handbrake, ghost storage, mobile layout, host/join track sync, 10-lap countdown, remote movement, reset and host transfer.');
