...
async function connectServer(value,quiet=false){
  ...
  try{
    ...
    const info=await res.json();if(info.app!=='apex-circuit'||info.version!==Physics.VERSION||info.protocol!==10)throw new Error('This server needs the Gamepad update.');
...
function updateAudio(p){if(!audio||!sound)return;const t=audio.context.currentTime,speed=Math.abs(p.speed),gear=Math.max(1,Math.ceil(speed/62)),rpm=55+(speed%62)*1.6+(p.throttle||0)*18;audio.engine.frequency.setTargetAtTime(rpm,t,.06);audio.harmonic.frequency.setTargetAtTime(rpm*.5,t,.06);audio.filter.frequency.setTargetAtTime(260+speed*2,t,.12);audio.gain.gain.setTargetAtTime(document.hidden?0:.012+(p.throttle||0)*.007,t,.1);}
let gamepadConnected=false,gamepadRecoverHeld=false;
function gamepadInput(){const pads=navigator.getGamepads?.()||[],pad=Array.from(pads).find(Boolean);if(!pad){if(gamepadConnected){gamepadConnected=false;$('gamepad-status').textContent='PS4 / PS5 CONTROLLER · READY';}return {steer:0,throttle:0,brake:0,handbrake:false};}if(!gamepadConnected){gamepadConnected=true;$('gamepad-status').textContent='CONTROLLER CONNECTED · '+pad.id.replace(/\s*\([^)]*\)/g,'').slice(0,28);toast('CONTROLLER CONNECTED · LEFT STICK + L2/R2');}const raw=pad.axes[0]||0,normal=Math.abs(raw)<.09?0:(Math.abs(raw)-.09)/.91,steer=Math.sign(raw)*Math.pow(normal,1.18),throttle=Math.max(0,pad.buttons[7]?.value||0),brake=Math.max(0,pad.buttons[6]?.value||0),handbrake=!!(pad.buttons[2]?.pressed||pad.buttons[0]?.pressed),recoverPressed=!!pad.buttons[3]?.pressed;if(recoverPressed&&!gamepadRecoverHeld)recover();gamepadRecoverHeld=recoverPressed;return {steer,throttle,brake,handbrake};}
