...
const setupFields=[['setup-downforce','downforce'],['setup-gearing','gearing'],['setup-compound','compound'],['setup-brake-bias','brakeBias'],['setup-differential','differential'],['setup-suspension','suspension'],['setup-anti-roll','antiRoll'],['setup-steering','steering'],['setup-tire-pressure','tirePressure']];
function readSetup(){return Physics.sanitizeSetup(Object.fromEntries(setupFields.map(([id,key])=>[key,$(id).value])));}
function renderSetupBase(){for(const [id,key] of setupFields)$(id).value=currentSetup[key];for(const [id,value] of [['brake-bias-value',currentSetup.brakeBias+'%'],['differential-value',currentSetup.differential+'%'],['suspension-value',currentSetup.suspension],['anti-roll-value',currentSetup.antiRoll],['steering-value',currentSetup.steering],['tire-pressure-value',currentSetup.tirePressure+' PSI']])$(id).textContent=value;const speed=55+(currentSetup.gearing==='long'?24:currentSetup.gearing==='short'?4:14)+(currentSetup.downforce==='low'?18:currentSetup.downforce==='high'?-8:5),corner=48+(currentSetup.downforce==='high'?25:currentSetup.downforce==='low'?-5:10)+(currentSetup.compound==='soft'?12:currentSetup.compound==='hard'?-3:5)+(currentSetup.suspension-50)*.18,stability=52+(12-Math.abs(currentSetup.brakeBias-58)*4)+(65-currentSetup.differential)*.25+(60-currentSetup.steering)*.15+(50-currentSetup.antiRoll)*.1;for(const [id,value] of [['setup-speed-meter',speed],['setup-corner-meter',corner],['setup-stability-meter',stability]])$(id).style.width=Math.max(12,Math.min(100,value))+'%';$('setup-status').textContent=currentSetup.downforce.toUpperCase()+' / '+currentSetup.compound.toUpperCase();$('setup-advice').textContent=currentSetup.downforce==='high'?'High aero rewards commitment through fast corners.':currentSetup.downforce==='low'?'Low drag favors long straights; brake earlier for fast turns.':currentSetup.differential>65?'A locked differential gives strong exits but can slide the rear.':currentSetup.suspension<38?'Soft suspension improves kerb control and makes direction changes calmer.':'Balanced aero and mechanical grip suit mixed circuits.';document.querySelectorAll('[data-setup-preset]').forEach(b=>b.setAttribute('aria-pressed',String(JSON.stringify(Physics.sanitizeSetup(Physics.setupPresets[b.dataset.setupPreset]))===JSON.stringify(currentSetup))));}
function renderSetup(){renderSetupBase();const life={soft:45,medium:70,hard:94}[currentSetup.compound]-(currentSetup.tirePressure-23)*4;$('setup-tire-meter').style.width=Math.max(15,Math.min(100,life))+'%';const balance=(currentSetup.differential-55)*.3+(currentSetup.steering-50)*.25+(currentSetup.antiRoll-50)*.14-(currentSetup.downforce==='high'?3:currentSetup.downforce==='low'?-3:0);$('setup-balance').textContent=balance>8?'ROTATION':balance<-8?'UNDERSTEER':'NEUTRAL';}
const engineerSetups={harbor:{downforce:'high',gearing:'short',compound:'soft',brakeBias:59,differential:58,suspension:48,antiRoll:55,steering:57,tirePressure:22},alpine:{...Physics.setupPresets.balanced,downforce:'high',suspension:42,steering:55},sunset:{...Physics.setupPresets.race,downforce:'low',gearing:'long',antiRoll:58},metro:{...Physics.setupPresets.qualifying,downforce:'high',differential:45,steering:54},emerald:{...Physics.setupPresets.race,downforce:'balanced',antiRoll:55},thunder:{...Physics.setupPresets.race,downforce:'low',antiRoll:62},zenith:{...Physics.setupPresets.balanced,downforce:'high',gearing:'short',steering:60},aurora:{...Physics.setupPresets.race,downforce:'balanced',suspension:35,differential:40},sakura:{...Physics.setupPresets.balanced,downforce:'high',steering:58,antiRoll:57},marina:{...Physics.setupPresets.race,downforce:'low',gearing:'long'},volcano:{...Physics.setupPresets.qualifying,downforce:'high',suspension:40,differential:47}};
...
function updateTrackUI(){const track=Physics.getTrack(selectedTrack),copy=trackCopy[selectedTrack];document.documentElement.style.setProperty('--lime',track.accent);$('nav-track').textContent=track.name.toUpperCase();$('race-track-name').textContent=track.name.toUpperCase();$('track-stamp').textContent=track.name.toUpperCase()+' / 11';if(selectedMode)$('track-intro').textContent=copy.intro;$('track-length').innerHTML=(track.length/2000).toFixed(1)+'<span> KM</span>';$('track-character').textContent=copy.character;$('track-condition').textContent=track.subtitle;$('weather').firstChild.textContent=copy.weather;$('weather').querySelector('small').textContent=copy.temperature;document.querySelectorAll('[data-track]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.track===selectedTrack)));}
...
function setMode(mode){
  ...
  $('panel-description').textContent=mode==='practice'?'Engineer your car, save a ghost, and master all eleven circuits.':mode==='sprint'?'Mika, Jules, and Nova are waiting. Build your setup and choose a race distance.':'Connect to the same server, choose a circuit and setup, then share a six-character code.';
}
...
async function connectServer(value,quiet=false){
  ...
  try{
    ...
    const info=await res.json();if(info.app!=='apex-circuit'||info.version!==Physics.VERSION||info.protocol!==7)throw new Error('This server needs the Race Hub update.');
