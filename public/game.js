...
$('engineer-setup').insertAdjacentHTML('beforebegin','<div class="setup-car-visual" id="setup-car-visual"><div class="setup-blueprint">LIVE VEHICLE MODEL <b id="visual-diff">DIFF 55 / 45</b></div><div class="setup-floor"></div><div class="setup-sidepod setup-sidepod-left"></div><div class="setup-sidepod setup-sidepod-right"></div><div class="setup-suspension setup-suspension-front"></div><div class="setup-suspension setup-suspension-rear"></div><div class="setup-axle setup-front"><span id="visual-front-wing">FW 6 · 23.0 PSI</span></div><div class="setup-cockpit"><i class="setup-halo"></i><b>DR</b><small id="visual-platform">32 MM · −2.5°</small></div><div class="setup-axle setup-rear"><span id="visual-rear-wing">RW 6 · 22.0 PSI</span></div><div class="setup-flow setup-flow-left"></div><div class="setup-flow setup-flow-right"></div><div class="setup-load setup-load-front"><span>FRONT LOAD</span><i><em id="visual-front-load"></em></i></div><div class="setup-load setup-load-rear"><span>REAR LOAD</span><i><em id="visual-rear-load"></em></i></div></div><div class="setup-diagnostics"><span><small>TRACK MATCH</small><b id="setup-score">76%</b><i><em id="setup-match-meter"></em></i></span><span><small>PACE FOCUS</small><b id="setup-focus">BALANCED</b></span><span><small>STINT WINDOW</small><b id="setup-stint">9 LAPS</b></span><span><small>TYRE WINDOW</small><b id="setup-tyre-window">F 23.0 · R 22.0</b></span></div>');
...
document.querySelector('.track-picker').insertAdjacentHTML('beforeend','<button data-track="obsidian" aria-pressed="false"><span>12</span><strong>Obsidian Pass</strong><small>Expert · Canyon</small></button><button data-track="titan" aria-pressed="false"><span>13</span><strong>Titan Ridge</strong><small>Expert · Alpine</small></button><button data-track="vesper" aria-pressed="false"><span>14</span><strong>Vesper Coast</strong><small>Endurance · Twilight</small></button><button data-track="copper" aria-pressed="false"><span>15</span><strong>Copper Works</strong><small>Sprint · Industrial</small></button><button data-track="lotus" aria-pressed="false"><span>16</span><strong>Lotus Delta</strong><small>Endurance · Night Delta</small></button>');
$('car-setup').insertAdjacentHTML('beforeend','<div class="advanced-setup"><div class="eyebrow">AERODYNAMIC & TYRE PLATFORM</div><div class="setup-grid"><label>FRONT WING <output id="front-wing-value">6</output><input id="setup-front-wing" type="range" min="1" max="11" value="6"></label><label>REAR WING <output id="rear-wing-value">6</output><input id="setup-rear-wing" type="range" min="1" max="11" value="6"></label><label>RIDE HEIGHT <output id="ride-height-value">32 MM</output><input id="setup-ride-height" type="range" min="25" max="45" value="32"></label><label>CAMBER <output id="camber-value">−2.5°</output><input id="setup-camber" type="range" min="15" max="35" value="25"></label><label>FRONT TOE <output id="front-toe-value">0.04°</output><input id="setup-front-toe" type="range" min="0" max="10" value="4"></label><label>REAR TOE <output id="rear-toe-value">0.10°</output><input id="setup-rear-toe" type="range" min="4" max="20" value="10"></label><label>REAR TYRE PRESSURE <output id="rear-tire-pressure-value">22.0 PSI</output><input id="setup-rear-tire-pressure" type="range" min="20" max="27" step="0.1" value="22"></label><label>BRAKE PRESSURE <output id="brake-pressure-value">95%</output><input id="setup-brake-pressure" type="range" min="80" max="100" value="95"></label><label>BRAKE COOLING <output id="brake-cooling-value">55%</output><input id="setup-brake-cooling" type="range" min="20" max="100" value="55"></label><label>ENGINE BRAKING <output id="engine-braking-value">50%</output><input id="setup-engine-braking" type="range" min="20" max="80" value="50"></label><label>ENGINE MODE<select id="setup-engine-mode"><option value="conserve">Conserve · tyre life</option><option value="standard" selected>Standard · balanced</option><option value="attack">Attack · peak power</option></select></label><label>FUEL LOAD <output id="fuel-load-value">45 KG</output><input id="setup-fuel-load" type="range" min="10" max="100" value="45"></label></div></div>');
...
const setupFields=[['setup-downforce','downforce'],['setup-front-wing','frontWing'],['setup-rear-wing','rearWing'],['setup-ride-height','rideHeight'],['setup-camber','camber'],['setup-front-toe','frontToe'],['setup-rear-toe','rearToe'],['setup-front-tire-pressure','frontTirePressure'],['setup-rear-tire-pressure','rearTirePressure'],['setup-brake-pressure','brakePressure'],['setup-brake-cooling','brakeCooling'],['setup-engine-braking','engineBraking'],['setup-engine-mode','engineMode'],['setup-fuel-load','fuelLoad'],['setup-gearing','gearing'],['setup-compound','compound'],['setup-brake-bias','brakeBias'],['setup-differential','differential'],['setup-off-throttle-differential','offThrottleDifferential'],['setup-suspension','suspension'],['setup-anti-roll','antiRoll'],['setup-steering','steering']];
function readSetup(){return Physics.sanitizeSetup(Object.fromEntries(setupFields.map(([id,key])=>[key,$(id).value])));}
function renderSetupBase(){for(const [id,key] of setupFields)$(id).value=currentSetup[key];for(const [id,value] of [['front-wing-value',currentSetup.frontWing],['rear-wing-value',currentSetup.rearWing],['ride-height-value',currentSetup.rideHeight+' MM'],['camber-value','−'+(currentSetup.camber/10).toFixed(1)+'°'],['front-toe-value',(currentSetup.frontToe/100).toFixed(2)+'°'],['rear-toe-value',(currentSetup.rearToe/100).toFixed(2)+'°'],['front-tire-pressure-value',currentSetup.frontTirePressure.toFixed(1)+' PSI'],['rear-tire-pressure-value',currentSetup.rearTirePressure.toFixed(1)+' PSI'],['brake-pressure-value',currentSetup.brakePressure+'%'],['brake-cooling-value',currentSetup.brakeCooling+'%'],['engine-braking-value',currentSetup.engineBraking+'%'],['fuel-load-value',currentSetup.fuelLoad+' KG'],['brake-bias-value',currentSetup.brakeBias+'%'],['differential-value',currentSetup.differential+'%'],['off-throttle-differential-value',currentSetup.offThrottleDifferential+'%'],['suspension-value',currentSetup.suspension],['anti-roll-value',currentSetup.antiRoll],['steering-value',currentSetup.steering]])$(id).textContent=value;const wingDrag=(currentSetup.frontWing+currentSetup.rearWing-12)*1.7,power=currentSetup.engineMode==='attack'?8:currentSetup.engineMode==='conserve'?-7:0,fuelPenalty=(currentSetup.fuelLoad-10)*.16,toeDrag=(currentSetup.frontToe+currentSetup.rearToe)*.22,coolingDrag=(currentSetup.brakeCooling-50)*.035,speed=69+(currentSetup.gearing==='long'?20:currentSetup.gearing==='short'?-2:10)+(currentSetup.downforce==='low'?12:currentSetup.downforce==='high'?-7:3)-wingDrag+power-fuelPenalty-toeDrag-coolingDrag,corner=45+(currentSetup.downforce==='high'?15:currentSetup.downforce==='low'?-4:7)+(currentSetup.compound==='soft'?10:currentSetup.compound==='hard'?-2:4)+currentSetup.frontWing*2+(currentSetup.suspension-50)*.12+currentSetup.frontToe*.65+(50-currentSetup.offThrottleDifferential)*.08-currentSetup.fuelLoad*.06-Math.abs(currentSetup.frontTirePressure-23)*1.8,stability=47+currentSetup.rearWing*2.2+(12-Math.abs(currentSetup.brakeBias-58)*4)+(65-currentSetup.differential)*.2+(currentSetup.offThrottleDifferential-45)*.16+(50-currentSetup.antiRoll)*.08+currentSetup.rearToe*.35-Math.abs(currentSetup.rideHeight-32)*.8-Math.abs(currentSetup.rearTirePressure-22)*1.5;for(const [id,value] of [['setup-speed-meter',speed],['setup-corner-meter',corner],['setup-stability-meter',stability]])$(id).style.width=Math.max(12,Math.min(100,value))+'%';$('setup-status').textContent=currentSetup.engineMode.toUpperCase()+' · W'+currentSetup.frontWing+'/'+currentSetup.rearWing;$('setup-advice').textContent=Math.abs((currentSetup.frontTirePressure-currentSetup.rearTirePressure)-1)>1.5?'A large front-to-rear pressure split can make grip and temperature balance unpredictable.':currentSetup.offThrottleDifferential<32?'A very open coast differential sharpens corner entry but makes lift-off rotation harder to catch.':currentSetup.offThrottleDifferential>68?'A locked coast differential stabilizes braking but can resist rotation into slow corners.':currentSetup.brakeCooling<35?'Closed brake ducts reduce drag but risk brake fade over a long race.':currentSetup.brakePressure>98?'Maximum brake pressure shortens stops but raises lock-up risk on grass and kerbs.':currentSetup.engineBraking>68?'High engine braking rotates the rear on corner entry; release the throttle smoothly.':currentSetup.rearToe>16?'Extra rear toe improves high-speed stability with more drag and tyre wear.':currentSetup.engineMode==='attack'?'Attack mode unlocks peak power for qualifying; manage tyre temperature over a race distance.':currentSetup.fuelLoad>75?'A heavy fuel load is stable over distance but slows braking, turn-in, and acceleration.':currentSetup.frontWing>currentSetup.rearWing+2?'Front-biased aero gives sharp turn-in but a nervous rear at speed.':currentSetup.rearWing>currentSetup.frontWing+2?'Rear-biased aero adds traction and stability but can create understeer.':'A balanced aero, tyre, and mechanical platform keeps the car predictable through a full stint.';document.querySelectorAll('[data-setup-preset]').forEach(b=>b.setAttribute('aria-pressed',String(JSON.stringify(Physics.sanitizeSetup(Physics.setupPresets[b.dataset.setupPreset]))===JSON.stringify(currentSetup))));}
function renderSetup(){
  renderSetupBase();
  const averagePressure=(currentSetup.frontTirePressure+currentSetup.rearTirePressure)/2,life={soft:45,medium:70,hard:94}[currentSetup.compound]-Math.max(0,averagePressure-22.5)*4-Math.abs(currentSetup.frontTirePressure-currentSetup.rearTirePressure-1)*2-(currentSetup.camber-25)*.9-(currentSetup.frontToe+currentSetup.rearToe)*.18;
  $('setup-tire-meter').style.width=Math.max(15,Math.min(100,life))+'%';
  const balance=(currentSetup.frontWing-currentSetup.rearWing)*2.8+(currentSetup.differential-55)*.22+(45-currentSetup.offThrottleDifferential)*.18+(currentSetup.steering-50)*.2+(currentSetup.antiRoll-50)*.12+currentSetup.frontToe*.35-currentSetup.rearToe*.18+(currentSetup.engineBraking-50)*.08+(currentSetup.rearTirePressure-currentSetup.frontTirePressure+1)*1.3;
  $('setup-balance').textContent=balance>8?'OVERSTEER':balance<-8?'UNDERSTEER':'NEUTRAL';
  const visual=$('setup-car-visual'),frontLoad=Math.max(18,Math.min(100,34+currentSetup.frontWing*5+(currentSetup.brakeBias-58)*3)),rearLoad=Math.max(18,Math.min(100,36+currentSetup.rearWing*5+(55-currentSetup.differential)*.25));
  visual.style.setProperty('--front-wing',currentSetup.frontWing*3+'px');visual.style.setProperty('--rear-wing',currentSetup.rearWing*3+'px');visual.style.setProperty('--front-toe',(currentSetup.frontToe-4)*.7+'deg');visual.style.setProperty('--rear-toe',(currentSetup.rearToe-10)*.32+'deg');visual.style.setProperty('--balance',Math.max(-6,Math.min(6,balance*.5))+'deg');visual.style.setProperty('--ride-height',(currentSetup.rideHeight-25)/20);
  $('visual-front-wing').textContent='FW '+currentSetup.frontWing+' · '+currentSetup.frontTirePressure.toFixed(1)+' PSI';$('visual-rear-wing').textContent='RW '+currentSetup.rearWing+' · '+currentSetup.rearTirePressure.toFixed(1)+' PSI';$('visual-platform').textContent=currentSetup.rideHeight+' MM · −'+(currentSetup.camber/10).toFixed(1)+'°';$('visual-diff').textContent='DIFF '+currentSetup.differential+' / '+currentSetup.offThrottleDifferential;$('visual-front-load').style.width=frontLoad+'%';$('visual-rear-load').style.width=rearLoad+'%';$('setup-tyre-window').textContent='F '+currentSetup.frontTirePressure.toFixed(1)+' · R '+currentSetup.rearTirePressure.toFixed(1);
  const target=Physics.sanitizeSetup(engineerSetups[selectedTrack]||Physics.setupPresets.balanced),diff=Math.abs(currentSetup.frontWing-target.frontWing)*2.2+Math.abs(currentSetup.rearWing-target.rearWing)*2.2+Math.abs(currentSetup.rideHeight-target.rideHeight)*.7+Math.abs(currentSetup.differential-target.differential)*.18+Math.abs(currentSetup.offThrottleDifferential-target.offThrottleDifferential)*.16+Math.abs(currentSetup.suspension-target.suspension)*.12+Math.abs(currentSetup.antiRoll-target.antiRoll)*.1+Math.abs(currentSetup.steering-target.steering)*.15+Math.abs(currentSetup.frontToe-target.frontToe)*.3+Math.abs(currentSetup.rearToe-target.rearToe)*.2+Math.abs(currentSetup.frontTirePressure-target.frontTirePressure)*1.1+Math.abs(currentSetup.rearTirePressure-target.rearTirePressure)*1.1+Math.abs(currentSetup.brakePressure-target.brakePressure)*.2+Math.abs(currentSetup.brakeCooling-target.brakeCooling)*.08+Math.abs(currentSetup.engineBraking-target.engineBraking)*.08+(currentSetup.downforce===target.downforce?0:7)+(currentSetup.gearing===target.gearing?0:5),score=Math.max(35,Math.round(100-diff)),focus=currentSetup.downforce==='low'||currentSetup.gearing==='long'?'STRAIGHT SPEED':balance>7?'ROTATION':balance<-7?'STABILITY':'BALANCED PACE',stint=Math.max(2,Math.round(life/8.5*(currentSetup.engineMode==='conserve'?1.15:currentSetup.engineMode==='attack'?.82:1)));
  $('setup-score').textContent=score+'%';$('setup-match-meter').style.width=score+'%';$('setup-focus').textContent=focus;$('setup-stint').textContent=stint+' LAPS';
}
const engineerSetups={harbor:{...Physics.setupPresets.balanced,downforce:'high',frontWing:9,rearWing:8,gearing:'short',compound:'soft',brakeBias:59,differential:58,suspension:48,antiRoll:55,steering:57,tirePressure:22},alpine:{...Physics.setupPresets.balanced,downforce:'high',frontWing:8,rearWing:9,rideHeight:35,suspension:42,steering:55},sunset:{...Physics.setupPresets.race,downforce:'low',frontWing:3,rearWing:4,gearing:'long',antiRoll:58},metro:{...Physics.setupPresets.qualifying,downforce:'high',frontWing:11,rearWing:10,rideHeight:36,differential:45,steering:54,frontToe:7,rearToe:14,brakePressure:98,engineBraking:70},emerald:{...Physics.setupPresets.race,downforce:'balanced',frontWing:7,rearWing:7,antiRoll:55},thunder:{...Physics.setupPresets.race,downforce:'low',frontWing:4,rearWing:5,antiRoll:62},zenith:{...Physics.setupPresets.balanced,downforce:'high',frontWing:10,rearWing:9,rideHeight:34,gearing:'short',steering:60,frontToe:6,rearToe:15,brakePressure:97,engineBraking:68},aurora:{...Physics.setupPresets.race,downforce:'balanced',frontWing:7,rearWing:9,rideHeight:37,suspension:35,differential:40,frontToe:3,rearToe:17,brakePressure:88,engineBraking:35},sakura:{...Physics.setupPresets.balanced,downforce:'high',frontWing:9,rearWing:9,steering:58,antiRoll:57},marina:{...Physics.setupPresets.race,downforce:'balanced',frontWing:6,rearWing:7,rideHeight:34,gearing:'long',camber:27},volcano:{...Physics.setupPresets.qualifying,downforce:'high',frontWing:10,rearWing:10,rideHeight:37,suspension:40,differential:47},obsidian:{...Physics.setupPresets.qualifying,downforce:'high',frontWing:11,rearWing:10,rideHeight:36,camber:30,gearing:'short',suspension:38,differential:43,frontToe:7,rearToe:16,brakePressure:94,engineBraking:72},titan:{...Physics.setupPresets.race,downforce:'high',frontWing:10,rearWing:11,rideHeight:35,camber:29,brakeBias:59,differential:46,gearing:'short',suspension:39,antiRoll:44,steering:58,tirePressure:22},vesper:{...Physics.setupPresets.race,downforce:'balanced',frontWing:7,rearWing:9,rideHeight:34,camber:27,brakeBias:57,differential:44,gearing:'long',suspension:42,antiRoll:48,steering:54,tirePressure:23},copper:{...Physics.setupPresets.qualifying,downforce:'high',frontWing:9,rearWing:8,rideHeight:35,brakeBias:60,brakeCooling:74,differential:52,gearing:'short',suspension:45,antiRoll:57,steering:60,frontTirePressure:22.2,rearTirePressure:21.4},lotus:{...Physics.setupPresets.race,downforce:'high',frontWing:10,rearWing:10,rideHeight:38,brakeBias:56,brakeCooling:82,differential:38,gearing:'balanced',suspension:34,antiRoll:38,steering:52,frontTirePressure:21.8,rearTirePressure:20.9}};
engineerSetups.lotus={...Physics.setupPresets.race,downforce:'high',frontWing:11,rearWing:10,rideHeight:37,brakeBias:57,brakeCooling:76,differential:42,offThrottleDifferential:34,gearing:'short',suspension:36,antiRoll:43,steering:60,frontTirePressure:21.9,rearTirePressure:21.1};
...
trackCopy.copper={intro:'Thread a floodlit foundry district of turbine bends, hard braking zones, and a fast pipeline return.',character:'INDUSTRIAL SPRINT',weather:'⚙',temperature:'26° · TRACK 39°'};
trackCopy.lotus={intro:'Race the rebuilt Lotus Delta at night: three narrow river channels, linked switchbacks, and a flat-out levee return.',character:'NIGHT DELTA ENDURANCE',weather:'☾',temperature:'24° · TRACK 28°'};
function updateTrackUI(){const track=Physics.getTrack(selectedTrack),copy=trackCopy[selectedTrack];document.documentElement.style.setProperty('--lime',track.accent);$('nav-track').textContent=track.name.toUpperCase();$('race-track-name').textContent=track.name.toUpperCase();$('track-stamp').textContent=track.name.toUpperCase()+' / 16';if(selectedMode)$('track-intro').textContent=copy.intro;$('track-length').innerHTML=(track.length/2000).toFixed(1)+'<span> KM</span>';$('track-character').textContent=copy.character;$('track-condition').textContent=track.subtitle;$('weather').firstChild.textContent=copy.weather;$('weather').querySelector('small').textContent=copy.temperature;document.querySelectorAll('[data-track]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.track===selectedTrack)));}
...
function renderHistory(){
  ...
  for(const [index,lap] of lapHistory.entries()){const row=document.createElement('article');row.className='telemetry-row';row.classList.toggle('invalid',!lap.valid);const telemetry=lap.telemetry||{},setup=lap.setup||{};
    ...
    const setupLine=document.createElement('p');setupLine.className='telemetry-setup';setupLine.textContent=(lap.driver||'Driver')+' · '+String(setup.compound||'medium').toUpperCase()+' · WINGS '+(setup.frontWing??'—')+'/'+(setup.rearWing??'—')+' · PSI '+(setup.frontTirePressure??setup.tirePressure??'—')+'/'+(setup.rearTirePressure??setup.tirePressure??'—')+' · '+(lap.limits||0)+' LIMIT STRIKES';row.append(setupLine);container.append(row);
  }
}
...
async function connectServer(value,quiet=false){
  ...
  try{
    ...
    const info=await res.json();if(info.app!=='apex-circuit'||info.version!==Physics.VERSION||info.protocol!==16)throw new Error('This server needs the Delta Cockpit update.');
...
const cameraModes=['overview','chase','cinematic','onboard'];
...
function hud(p,players,time){
  ...
  $('gear').textContent=p.speed<-2?'R':Math.abs(p.speed)<3?'N':Math.min(6,Math.ceil(Math.abs(p.speed)/62));$('rev').style.width=Math.min(100,Math.abs(p.speed)/340*100)+'%';$('throttle-bar').style.width=Math.round((p.throttle||0)*100)+'%';$('brake-bar').style.width=Math.round((p.brake||0)*100)+'%';document.querySelector('.speed-cell').classList.toggle('braking',(p.brake||0)>.08);
  ...
  $('tire-temp').textContent='F'+Math.round(p.frontTireTemp??p.tireTemp??72)+'° R'+Math.round(p.rearTireTemp??p.tireTemp??72)+'° · '+Math.round(p.tireWear??100)+'%';$('tire-temp').style.color=p.tireWear<30||Math.max(p.frontTireTemp??72,p.rearTireTemp??72)>108?'#ff9675':p.tireTemp>82?'var(--lime)':'';$('brake-temp').textContent=Math.round(p.brakeTemp??180)+'°C';$('brake-temp').style.color=p.brakeTemp>850?'#ff745f':p.brakeTemp>600?'#ffb066':'';$('damage').textContent=Math.round(p.damage||0)+'%';$('damage').style.color=p.damage>50?'#ff9675':'';$('limits').textContent=(p.trackLimits||0)+(p.blackWhite?' · B/W':p.penalty?' · +'+p.penalty/1000+'s':' STRIKES');$('limits').style.color=p.blackWhite||p.penalty?'#ffb066':'';
  $('car-status').textContent=p.impact>20?'IMPACT':p.wheelsOut===4?'ALL WHEELS OUT':p.wheelsOut?'WHEELS ON WHITE LINE':p.brakeTemp>850?'BRAKES HOT':p.abs?'ABS ACTIVE':p.tractionControl?'TRACTION CONTROL':p.tireTemp>108?'TYRES HOT':'TRACK CLEAR';
