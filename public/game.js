...
function setMode(mode){
  ...
  $('panel-description').textContent=mode==='practice'?'Engineer your car, save a ghost, and master all seven circuits.':mode==='sprint'?'Mika, Jules, and Nova are waiting. Build your setup and choose a race distance.':'Connect to the same server, choose a circuit and setup, then share a six-character code.';
}
...
async function connectServer(value,quiet=false){
  ...
  try{
    ...
    const info=await res.json();if(info.app!=='apex-circuit'||info.version!==Physics.VERSION||info.protocol!==6)throw new Error('This server needs the Evolution update.');
