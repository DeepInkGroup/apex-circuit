...
function setMode(mode){
  ...
  $('panel-description').textContent=mode==='practice'?'Engineer your car, read the telemetry, and master all fourteen circuits.':mode==='sprint'?'Mika, Jules, and Nova are waiting. Build your setup and choose a race distance.':'Connect to the same server, choose a circuit and setup, then share a six-character code.';
}
...
async function connectServer(value,quiet=false){
  ...
  try{
    ...
    const info=await res.json();if(info.app!=='apex-circuit'||info.version!==Physics.VERSION||info.protocol!==11)throw new Error('This server needs the Grand Tour update.');
