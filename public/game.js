...
async function connectServer(value,quiet=false){
  ...
  try{
    ...
    const info=await res.json();if(info.app!=='apex-circuit'||info.version!==Physics.VERSION||info.protocol!==9)throw new Error('This server needs the Qualifying update.');
