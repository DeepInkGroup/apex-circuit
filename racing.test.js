...
for(const trackId of ['alpine','sunset','metro','emerald','thunder','zenith','aurora','sakura','marina','volcano'])test('AI field completes three clean laps at '+trackId,()=>{
  ...
  assert.equal(race.status,'finished');assert.equal(race.trackId,trackId);for(const p of race.players){assert.equal(p.lap,3,p.name+' finishes');assert.equal(p.trackId,trackId);assert.equal(p.lastValid,true,p.name+' finishes a clean final lap');assert.equal(p.penalty,0,p.name+' has no limits penalty');}
});
